...
**Botensilimab as independent clinical validation.** Recent phase 1/2 data for botensilimab (anti-CTLA-4 with engineered Fc tail) combined with balstilimab (anti-PD-1) provides independent clinical evidence consistent with the gate-jamming framework (Syn et al., ASCO 2023; Hamid et al., ASCO 2024). Bot/Bal achieves approximately 20% response rate and 42% two-year overall survival in MSS CRC — the first immunotherapy combination with meaningful activity in this population, where standard ICI achieves near-zero durable responses. The mechanism is framework-predicted: botensilimab's engineered Fc tail directly activates innate immune cells (NK cells, macrophages, dendritic cells) through FcγR binding, bypassing the silenced cGAS-STING innate alarm entirely. Rather than restoring the jammed gate, Bot/Bal routes around it via a different door. That this bypass strategy produces responses specifically in MSS CRC — and not in populations where cGAS-STING is constitutively active — is indirect clinical evidence that the innate priming axis is the rate-limiting step in this tumor type. The BATTMAN Phase 3 trial, now enrolling, will test whether this effect extends to randomized endpoints.

**The hepatic macrophage barrier and a fourth therapeutic layer.** Bot/Bal trials exclude patients with active liver metastases, and the same regimen achieves essentially zero responses when liver mets are present. The mechanism reveals an additional immune evasion layer operating at organ scale: hepatic macrophages constitutively express FasL, and activated T cells entering hepatic vasculature undergo FasL-mediated apoptosis before reaching the tumor. This organ-scale evasion is mechanistically distinct from gate-jamming: gate-jamming suppresses innate priming (Step 2), while hepatic FasL macrophages eliminate generated T cells in transit (Step 3). Bot/Bal solves Step 2; no current regimen reliably solves Step 3. This Step 3 barrier represents a third level of the belt-and-suspenders immune evasion architecture: at the molecular scale, VDAC1 gate-jamming prevents mtDNA release and TREX1 erases any that leaks; at the cellular scale, cGAS-STING suppression prevents innate alarm propagation and ENPP1 degrades extracellular cGAMP; at the organ scale, tumor-local immunosuppression (Tregs, PD-L1) prevents T cell function and hepatic FasL macrophages execute escaped T cells systemically. For MSS CRC patients with liver involvement, a four-layer intervention — gate-opener, eraser inhibitor, checkpoint blockade, and hepatic macrophage conditioning (liver-directed SBRT or FasL pathway inhibition) — may be required.
